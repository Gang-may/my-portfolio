document.addEventListener("DOMContentLoaded", () => {
      ...
      scales: {
        r: {
          ...
          pointLabels: {
            color: textColor,
            font: {
              family: "'Inter', 'Pretendard Variable', sans-serif",
              size: 12,
              weight: 600,
            },
          },
  ...
  // 2. Cognitive Chart
  const ctxCognitive = document
    .getElementById("cognitiveChart")
    .getContext("2d");
  const dataCognitive = {
  ...
  const modalData = {
    "modal-1": {
      ...
      content:
        "특정 키워드(휴먼노이드)와 관련된 뉴스 기사를 자동으로 수집(Crawling)하여 엑셀 파일로 저장하고, 주요 키워드의 빈도를 분석하는 프로젝트입니다.<br><br><strong>배운 점:</strong> 웹 구조(HTML/DOM)의 이해 및 동적 페이지에서의 데이터 추출 한계점 인식.",
    ...
    "modal-2": {
      ...
      content:
        "방위사업청의 공공데이터를 활용하여 군수품 입찰 현황을 시각화한 대시보드입니다. 지역별, 품목별 입찰 현황을 한눈에 파악할 수 있도록 구성했습니다.<br><br><strong>배운 점:</strong> 대용량 데이터의 정제 과정과 사용자 친화적인 대시보드 UI/UX 설계 및 시각적 전달력 강화 방법.",
    ...
    "modal-3": {
      ...
      content:
        "주어진 기업 현장 데이터를 분석하여 문제 해결 모델을 개발하는 경진대회 참가작입니다. 불균형 데이터셋에 대한 처리와 예측 모델 성능 개선에 집중했습니다.<br><br><strong>배운 점:</strong> 실무 데이터가 가진 다양한 결측치와 이상치를 다루는 방법 및 머신러닝 교차검증(Cross Validation)의 중요성.",
    ...
    "modal-4": {
      ...
      content:
        "제주도의 관광 데이터(유동인구, 소비, 날씨 등)를 결합하여 제주 관광객의 동선 및 소비 패턴을 분석하고 인사이트를 도출한 공모전 출품작입니다.<br><br><strong>배운 점:</strong> 서로 다른 성격의 데이터셋을 병합하는 기술과 지리적(공간적) 데이터 분석 및 시각화 역량 향상.",
    ...
    "modal-5": {
      ...
      content:
        "전장 환경의 실시간 지리정보(GIS)와 AI 분석을 결합하여, 지휘관이 신속하고 정확한 전술적 의사결정을 내릴 수 있도록 돕는 시뮬레이션 및 의사결정 지원 시스템 기획/개발 프로젝트입니다.<br><br><strong>배운 점:</strong> AI 추론 결과를 지도 위에 실시간으로 매핑하는 기술력과 복잡한 센서/데이터 인터랙션을 설계하는 방법론.",
    ...
    "modal-6": {
      ...
      content:
        "해외(호주)에서 한국산 배(Bae) 음료를 숙취 해소 음료로 브랜딩하여 성공한 'Bae Juice' 사례를 심층 분석하고 시사점을 도출한 리포트입니다.<br><br><strong>배운 점:</strong> 현지 문화적 특성을 고려한 포지셔닝(Positioning) 전략의 중요성과 스토리텔링 마케팅의 위력.",
    ...
    "modal-7": {
      ...
      content:
        "인건비 상승과 언택트 트렌드로 부상한 무인점포 시장의 현황 및 문제점(도난, 기술 소외 계층 등)을 분석하고, 이를 극복하기 위한 장기적 확산 전략을 제안했습니다.<br><br><strong>배운 점:</strong> 사회 변화에 따른 리테일 비즈니스의 구조적 변화 이해 및 한계점에 대한 창의적 타개책 마련.",
    ...
    "modal-8": {
      ...
      content:
        "기존의 낙후되거나 특정 이미지에 고착화된 아파트 브랜드 '사랑으로 부영'의 문제점을 진단하고, 핵심 가치를 재정의하여 타겟 고객층에 소구하는 새로운 브랜드 아이덴티티와 커뮤니케이션 전략 모델을 수립한 기획서입니다.<br><br><strong>배운 점:</strong> 브랜드 진단 프로세스와 기존 자산(Brand Equity)을 활용하면서도 새로운 이미지를 부여하는 리브랜딩 프로세스.",
    ...
    "modal-9": {
      ...
      content:
        "생산성 툴 'Notion(노션)'을 아직 적극적으로 사용하지 않는 대학생 및 주니어 직장인을 타겟으로, 그들의 페인포인트(Pain-point)를 자극하고 솔루션을 제시하는 디지털 광고 캠페인 소재와 집행 전략을 기획했습니다.<br><br><strong>배운 점:</strong> 타겟 세분화에 따른 맞춤형 메시지(광고 카피) 도출 방법론과 효과적인 미디어 매체 선정 기준.",
...
class Star {
  ...
  draw() {
    ...
    ctx.fillStyle =
      theme === "dark" ? "rgba(255, 255, 255, 0.8)" : "rgba(15, 23, 42, 0.3)";
    ctx.beginPath();
  ...
  update() {
    ...
    if (mouse.x != null && mouse.y != null) {
      ...
      if (distance < maxDistance) {
        ...
        let tgtX =
          this.x + Math.cos(spiralAngle) * 8 * force + dx * pullStrength * 0.15;
        let tgtY =
          this.y + Math.sin(spiralAngle) * 8 * force + dy * pullStrength * 0.15;
